                ...
                <div className="flex items-center justify-center gap-1 md:gap-2 flex-wrap">
                    {filterButtons.map(({ key, label, icon: Icon }) => (
                         <button 
                            ...
                            className={`px-1.5 md:px-4 py-1.5 md:py-2 rounded-lg text-xs md:text-sm font-bold flex items-center gap-1 md:gap-2 transition-all duration-200 ${filter === key ? 'bg-electric-500 text-cloud-white' : 'bg-white/15 border border-white/25 text-system-grey hover:bg-white/25 hover:border-white/45 backdrop-blur-sm'}`}
                        >
